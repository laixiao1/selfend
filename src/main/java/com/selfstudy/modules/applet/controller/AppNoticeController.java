...
public class AppNoticeController {
    ...
    public R getSeatByRoom(@RequestParam(value = "id") Long id,
                           @RequestParam(value = "date",required = false)String date,
                           @RequestParam(value = "time",required = false)String time){
        System.out.println(id);
        System.out.println(date);
        System.out.println(time);
        List<BasSeatEntity> roomEntities = basSeatService.getSeatByRoom(id,date,time);
        return R.ok().put("data", roomEntities);
    }
    ...
    public R cancel(Long id) {
        try {
            Boolean result = basAppointmentService.cancel(id);
            if (result) {
                return R.ok();
            }
